		...
		try {
			if ($this->_msgType != 'image') {
				...
				if ($this->_msgType == 'text') {
					...
					if(strpos($contents, "想你了") !== false) {
						$contentStr = FF_HINT_XIANGNILE;
					}
					if(strpos($contents, "打飞机") !== false) {
						$resultStr = sprintf ( SUCC_TPL_DAFEIJI, $this->_fromUserName, $this->_toUserName, $this->_time);
						return $resultStr;
